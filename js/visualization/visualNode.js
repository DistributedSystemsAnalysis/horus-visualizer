...
function VisualNode(node) {
    ...
    if (this.isStart()) {
        ...
        if (this.isLog()) {
            ...
            this.$rect.attr({
                "width": Global.TYPE_LOG_NODE_SIZE,
            ...
            // use a circle for non start nodes whose type isn't LOG
...
VisualNode.prototype.setX = function(newX) {
    if (this.isStart())
        this.translateX = newX - (Global.HOST_SIZE / 2);
    else if (this.isLog())
        this.translateX = newX - (Global.TYPE_LOG_NODE_SIZE / 2);
    else
        this.translateX = newX;

    this.x = newX;
    this.$svg.attr("transform", "translate(" + this.translateX + "," + this.translateY + ")");
};
...
VisualNode.prototype.setY = function(newY) {
    this.translateY = this.isLog() ? (newY - (Global.TYPE_LOG_NODE_SIZE / 2)) : newY;
...
VisualNode.prototype.setStrokeWidth = function(newStrokeWidth) {
    ...
    this.$diamond.attr("stroke-width", newStrokeWidth + "px");
};
...
/**
 * Determines is this VisualNode represents an event of type LOG. Events of type
 * LOG will be drawn differentely from other nodes.
 */
VisualNode.prototype.isLog = function() {
    const node = this.node;
    return (node.getLogEventCount() == 1) && (['LOG', 'FSYNC'].includes(node.getFirstLogEvent().getText()));
}
